"""
Fetches and stores BLOG metafield definitions from Shopify.
Mirrors products/metafield_defs.py exactly but for the BLOG owner type.
Output: {store_dir}/blog_metafield_defs.json
"""
...
query metafieldDefs($cursor: String) {
  metafieldDefinitions(ownerType: BLOG, first: 250, after: $cursor) {
...
def fetch_and_store_blog_metafield_defs(store_dir):
    print('[BLOG METAFIELD DEFS] Fetching blog definitions...')
    all_defs = {}
    cursor = None
    ...
    path = os.path.join(store_dir, 'blog_metafield_defs.json')
    with open(path, 'w', encoding='utf-8') as f:
        json.dump(all_defs, f, indent=2, ensure_ascii=False)

    print(f'[BLOG METAFIELD DEFS] {len(all_defs)} definition(s) saved to {path}')
    return all_defs


def load_blog_metafield_defs(store_dir):
    path = os.path.join(store_dir, 'blog_metafield_defs.json')
